...
class WomanNPC(NPC):
    ...
    def ajustar_propina(self, direccion):
        if direccion == "arriba":
            ...
            self.mensaje = "eres un puto pobre"
    def confirmar_propina(self):
        if self.wallet:
            if self.wallet.gastar(self.propina_valor):
                self.modo_propina = False
                self.respuesta_activa = f"oh es muy amable mi lord, puedo darle otros servicios ¿si quiere ?"
                self.esperando_opcion = False
            else:
                self.mensaje_error = "No tienes suficiente dinero para dar esa propina"
        else:
            self.modo_propina = False
            self._resetear()
